...
class Model_Trainer:
    ...
    def train_skill_classification(self, path_models):
        ...
        # NLP Doc2Vec processing
        time_ini = time.time()
        # Bag of words + tf-idf
        bow_tfidf = self.nlp.Nlp_Doc2Vec(df_estrat['backward_text'].tolist(), path_models)
        time_end = time.time()
        ...
        y_val = clf.predict(self.nlp.Nlp_Doc2Vec(df_train['backward_text'].tolist(), path_models))
